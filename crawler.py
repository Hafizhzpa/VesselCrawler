...
while True:
    ...
    df_imo.to_excel("data/master_IMO_final.xlsx",index=False)
    ...
    df_final['Update Time'] = pd.to_datetime(df_final['Update Time'], format='%d/%m/%Y %H:%M')
    ...
    time_arrival = result['Arrival'][result["Status"] == "Schedule"].tolist()
    time_arrival_dt = [pd.to_datetime(arrival) for arrival in time_arrival]
    min_arrival = min(time_arrival_dt)
    time_now = datetime.now()
    next_run = (min_arrival - time_now).total_seconds() + 3600
    if next_run > 0:
        print(f"Next run on {time_now + timedelta(seconds=next_run)}")
        time.sleep(next_run)
